...
// Function to GET and display all loans
async function loadLoans() {
    const response = await fetch('/api/loans');
    const data = await response.json();
    
    const tableBody = document.getElementById('loan-table-body');
    tableBody.innerHTML = data.map(loan => `
        <tr>
            <td>${loan.borrower_name}</td>
            <td>₦${parseFloat(loan.principal).toLocaleString()}</td>
            <td>${loan.interest_rate}%</td>
            <td>
                <button onclick="deleteLoan(${loan.id})" class="btn-delete">Settled</button>
            </td>
        </tr>
    `).join('');
}